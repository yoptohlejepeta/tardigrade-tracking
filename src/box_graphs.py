...
for config in configs:
    ...
    for video_id in video_ids:
        ...
        mean_displacement = valid_data["displacement"].mean()
        video_means.append(mean_displacement)
        loginfo(f"  {video}: mean displacement = {mean_displacement:.4f}")

    boxplot_data.append(video_means)
...
for i, (data, color) in enumerate(zip(boxplot_data, colors)):
    ...
    ax.scatter(
        x, data, alpha=0.8, s=80, color=color, edgecolors="black", linewidth=1, zorder=3
    )

ax.set_ylabel("Mean Displacement", fontsize=12)
ax.set_title("Mean Displacement per Video by Group", fontsize=14)
ax.grid(axis="y", alpha=0.3, linestyle="--")

plt.tight_layout()
plt.savefig("displacement_boxplot.png", dpi=150)
plt.close()

loginfo("\nSaved: displacement_boxplot.png")
loginfo("Done!")
