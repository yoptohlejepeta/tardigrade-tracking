...
def track_objects_with_lookback(
    history, curr_regions: list, next_id, max_distance_percent
):
    if not curr_regions:
        return {}, next_id
    ...
    for frame_data in history:
        ...
        unmatched_curr_idx = [
            i for i in range(len(curr_regions)) if i not in matched_curr
        ]
...
def main():
    ...
    for frame_num, npy_file in enumerate(npy_files):
        ...
        history.append({"regions": regions, "id_map": id_map, "frame_num": frame_num})
        ...
        for region in regions:
            ...
            if track_id is None:
                loginfo(
                    f"Warning: region {region.label} has no track_id mapping in frame {frame_num}"
                )
                continue
